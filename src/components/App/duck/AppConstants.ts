import { AppActionTypesConstant } from '../AppTypes';

const appConstants: AppActionTypesConstant = {
    APP_LOGGED_IN: 'APP_LOGGED_IN',
    APP_LOGGED_OUT: 'APP_LOGGED_OUT',
    APP_USER_CHECK: 'APP_USER_CHECK',
    APP_USER_OFF: 'APP_USER_OFF',
    APP_AUTH_CHECK: 'APP_AUTH_CHECK',
    APP_AUTH_OFF: 'APP_AUTH_OFF',
    APP_KEY_CHECK: 'APP_KEY_CHECK',
    APP_USER_DATA_LOADED: 'APP_USER_DATA_LOADED',
    APP_PRIVATE_KEY_LOADED: 'APP_PRIVATE_KEY_LOADED',
};

export default appConstants;
